...
struct GlyphSchedulerTests {
    ...
    @Test("no trailing pause after the final glyph — the page settles on the last stroke")
    func noDeadTimeAtTheEnd() {
        let hand = HandProfile(penPace: 900, interGlyphPause: 0.02, wordPause: 0.12)
        let frames = GlyphStrokeScheduler.schedule([
            GlyphMetric(pathLength: 900),
            GlyphMetric(pathLength: 450, endsWord: true),
        ], hand: hand)

        #expect(frames[1].startTime == 1.02)
        #expect(GlyphStrokeScheduler.totalDuration(frames) == 1.52,
                "a trailing wordPause would report 1.64 and burn 0.12s of dead time")
        #expect(GlyphStrokeScheduler.totalDuration([]) == 0)
    }

    @Test("a nonpositive pen pace from a Book definition clamps instead of trapping")
    func handProfileClampsHostileValues() {
        let broken = HandProfile(penPace: 0, interGlyphPause: -5, wordPause: .nan)
        #expect(broken.penPace == HandProfile.minimumPenPace)
        #expect(broken.interGlyphPause == 0)
        #expect(broken.wordPause == 0)

        let frames = GlyphStrokeScheduler.schedule([GlyphMetric(pathLength: 10)], hand: broken)
        #expect(frames[0].duration.isFinite, "an infinite duration would hang the page forever")
    }

    @Test("a NaN or negative glyph length cannot poison the rest of the schedule")
    func glyphMetricClampsLength() {
        let hand = HandProfile(penPace: 100, interGlyphPause: 0, wordPause: 0)
        let frames = GlyphStrokeScheduler.schedule([
            GlyphMetric(pathLength: .nan),
            GlyphMetric(pathLength: -100),
            GlyphMetric(pathLength: 100),
        ], hand: hand)
        #expect(frames.allSatisfy { $0.startTime.isFinite && $0.duration >= 0 })
        #expect(frames[2].startTime == 0)
        #expect(GlyphStrokeScheduler.totalDuration(frames) == 1.0)
    }
