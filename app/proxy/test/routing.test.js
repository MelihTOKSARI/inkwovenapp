...
test('a routed provider streams deltas; prompt is injected server-side', async () => {
  ...
  const app = build({
    textProviderFactory: (book) =>
      async function* fake({ system, imageBase64 }) {
        seen.book = book.id;
        seen.system = system;
  ...
  assert.match(res.payload, /"modelID":"gemini-flash-lite"/);

  assert.equal(seen.book, 'oracle');
  assert.equal(seen.imageBase64, 'aW5r');
...
test('a provider failure degrades in-fiction, never a raw error', async () => {
  const app = build({
    textProviderFactory: () =>
...
test('routing table: gm/tutor get the heavy model, others the default', () => {
  ...
  assert.equal(findBook('gm').models.text, 'gpt-5-mini');
  assert.equal(findBook('tutor').models.text, 'gpt-5-mini');
  assert.equal(findBook('oracle').models.text, 'gemini-flash-lite');
});
